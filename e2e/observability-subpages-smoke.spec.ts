...
// Observability sub-pages + workflow dev-tools — never crawled by the audit
// (which only hits top-level /observability, /workflow). Fast render/no-error
// smoke, parallelized per-page.
const PAGES = [
...
for (const route of PAGES) {
  test(`smoke ${route}`, async ({ page }) => {
    ...
    console.log(`OBSSMOKE ${route} bodyLen=${bodyLen} errors=${errors.length}`);
