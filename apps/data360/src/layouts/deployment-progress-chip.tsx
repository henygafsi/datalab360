...
function statusTone(status: DeploymentStatus) {
  switch (status) {
    case 'RUNNING':
      return { ring: 'ring-blue-400', dot: 'bg-blue-500', bar: 'bg-blue-500' };
    ...
    case 'FAILED':
      return { ring: 'ring-rose-400', dot: 'bg-rose-500', bar: 'bg-rose-500' };
    case 'SUCCEEDED':
      return {
...
function StatusIcon({
  ...
  if (status === 'SUCCEEDED')
    return (
      <PiCheckCircleDuotone className={cn('text-emerald-500', className)} />
    );
  if (status === 'FAILED' || status === 'CANCELLED')
    return <PiXCircleDuotone className={cn('text-rose-500', className)} />;
  if (status === 'PENDING_APPROVAL')
    return (
      <PiClockCountdownDuotone className={cn('text-amber-500', className)} />
    );
  return (
    <PiRocketLaunchDuotone
      className={cn('animate-pulse text-blue-500', className)}
    />
  );
}
...
function DeployRow({ row }: { row: DeploymentRow }) {
  ...
  const failedSteps = Object.entries(row.errors_by_step || {}).filter(
    ([, errs]) => (errs || []).length > 0
  );
  return (
    <Link
            ...
            {failedSteps.length > 0 && (
              <>
                ...
                <span className="text-rose-500">
                  {failedSteps.length} error{failedSteps.length > 1 ? 's' : ''}
                </span>
              </>
            )}
          </div>
...
export default function DeploymentProgressChip() {
  ...
  const trigger = (
    ...
    >
      ...
      <div className="flex min-w-0 max-w-[180px] flex-col items-start">
        ...
        <div className="mt-0.5 flex items-center gap-1.5 text-[10px] text-slate-500">
          <span
            className={cn(
              ...
              tone.dot,
              headline.status === 'RUNNING' && 'animate-pulse'
            )}
          />
          <span>
            {stepLabel(headline.current_step)} ·{' '}
            {fmtDuration(headline.elapsed_ms)}
          </span>
        </div>
      </div>
      {items.length > 1 && (
  ...
  return (
    <Popover isOpen={open} setIsOpen={setOpen} placement="bottom-end">
      ...
      <Popover.Content className="z-[9999] w-[340px] p-2 shadow-lg">
