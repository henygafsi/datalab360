...
/**
 * ProjectsActionSurface — the agentic command surface for Projects: every
 * project-lifecycle capability rendered as a governed, verifiable action,
 * sourced live from the registry-in-tables catalog (GET /projects/actions), not
 * from hardcoded buttons. Mirrors the AI / E&D / Admin / Governance catalogs.
 *
 * Read-only actions (probe='get', GET, status='live') → "Run" executes inline.
 * Mutating live actions → "Open" jumps to the project dashboard where the full
 * control lives (mutations need a selected project + its confirmations, so
 * nothing mutating runs from the catalog directly). status='planned' rows are
 * roadmap capabilities with no endpoint yet — rendered as non-runnable chips so
 * the capability is VISIBLE without a phantom CTA to a 404.
 */
...
import {
  getProjectActions,
  verifyProjectActions,
  isProjectActionVerified,
  type ProjectAction,
  type ProjectActionCatalog,
} from '@/app/services/projects/actions';

const AREA_META: Record<string, { label: string; hint: string }> = {
  lifecycle: { label: 'Lifecycle', hint: 'Create, open, rename, delete, lock/unlock, 360' },
  contributors: { label: 'Contributors & sharing', hint: 'Grant owner / editor / viewer' },
  deployments: { label: 'Deployments', hint: 'Request, approve, reject, execute, rollback, runs' },
  comments: { label: 'Comments', hint: 'Threaded project discussion' },
  rls: { label: 'Row-access (RLS)', hint: 'Bind, list & unbind row-level policies from the project' },
  data_products: { label: 'Data products', hint: 'Register, publish as share, subscribe, refresh, lineage, consumers' },
  manifest: { label: 'Manifests & history', hint: 'Capability manifests, validation, event timeline' },
};
const AREA_ORDER = ['lifecycle', 'contributors', 'deployments', 'data_products', 'rls', 'comments', 'manifest'];
...
export default function ProjectsActionSurface() {
  ...
  const { data, loading, error, unavailable, refetch } = useCacheAwareQuery<ProjectActionCatalog>(
    getProjectActions,
    { cacheKeys: [CACHE_KEYS.PROJECTS] },
  );

  const runAction = useCallback(async (a: ProjectAction) => {
    setRuns((r) => ({ ...r, [a.action_id]: { running: true } }));
    trackFeatureClick(`proj_action_run_${a.action_id}`);
    try {
  ...
  const openArea = useCallback((a: ProjectAction) => {
    trackFeatureClick(`proj_action_open_${a.action_id}`);
    router.push(routes.project.dashboard);
    toast(`Open a project on the dashboard to complete "${a.label}".`, { icon: '→' });
  }, [router, trackFeatureClick]);

  const runVerify = useCallback(async () => {
    setVerifying(true);
    trackFeatureClick('proj_actions_verify');
    try {
      const res = await verifyProjectActions();
  ...
  const filtered = useMemo(() => {
    ...
    const rows = q
      ...
      : all;
    const byArea: Record<string, ProjectAction[]> = {};
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-900 dark:text-white">
            ...
            All project capabilities as actions
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {total} capabilities{data ? ` · ${data.live} live · ${data.planned} on the roadmap` : ''} — read-only ones run inline, changes open the dashboard, planned ones are labelled honestly
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search project capabilities — e.g. deploy, approve, rollback, share, publish, subscribe, RLS…"
      ...
      {unavailable && !loading && (
        <p role="status" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-400">
          The project action catalog isn&apos;t available on this account yet.
        </p>
      )}
      {error && !loading && (
