...
/**
 * CatalogActionSurface — the agentic command surface for the Sources/Catalog
 * page: every curator capability rendered as a governed, verifiable action,
 * sourced live from the registry-in-tables catalog (GET /catalog/actions),
 * not from hardcoded buttons. Mirrors the AI / E&D / Admin / Governance /
 * Projects / Workflow / Connect / DQ catalogs.
 *
 * Read-only actions (probe='get', GET, no path params) → "Run" executes
 * inline. Everything else → "Open" jumps to the Sources page where the full
 * control lives (mutations need a selected object + confirmations, so
...
import {
  getCatalogActions,
  verifyCatalogActions,
  isCatalogActionVerified,
  type CatalogAction,
  type CatalogActionCatalog,
} from '@/app/services/catalog/actions';

const AREA_META: Record<string, { label: string; hint: string }> = {
  explore: { label: 'Explore', hint: 'Overview, sources, account graph, timeline, tags, scores' },
  object: { label: 'Object intelligence', hint: '360, scores, clustering, history, profile, classify' },
  panel: { label: 'Smart panel', hint: 'Context, governance, lineage, ownership, ingestion reads' },
  product: { label: 'Data products', hint: 'Overview, assets, KPIs, AI modeling, publish' },
  kpi: { label: 'KPI registry', hint: 'Business metric definitions, create & validate' },
  ops: { label: 'Operations', hint: 'Recommendations, refresh lifecycle' },
};
const AREA_ORDER = ['explore', 'object', 'panel', 'product', 'kpi', 'ops'];
...
function ActionCard({
  ...
  run?: RunState;
  onRun: (a: CatalogAction) => void;
  onOpen: (a: CatalogAction) => void;
}) {
  const runnable = action.probe === 'get' && action.method === 'GET' && !action.path.includes('{');
  const verified = isCatalogActionVerified(action.verified_status);
  return (
...
export default function CatalogActionSurface() {
  ...
  const { data, loading, error, unavailable, refetch } = useCacheAwareQuery<CatalogActionCatalog>(
    getCatalogActions,
    { cacheKeys: [CACHE_KEYS.CATALOG] },
  );

  const runAction = useCallback(async (a: CatalogAction) => {
    setRuns((r) => ({ ...r, [a.action_id]: { running: true } }));
    trackFeatureClick(`catalog_action_run_${a.action_id}`);
    try {
  ...
  const openArea = useCallback((a: CatalogAction) => {
    trackFeatureClick(`catalog_action_open_${a.action_id}`);
    router.push('/sources');
    toast(`Use the Sources page to complete "${a.label}".`, { icon: '→' });
  }, [router, trackFeatureClick]);

  const runVerify = useCallback(async () => {
    setVerifying(true);
    trackFeatureClick('catalog_actions_verify');
    try {
      const res = await verifyCatalogActions();
  ...
  const filtered = useMemo(() => {
    ...
    const rows = q
      ...
      : all;
    const byArea: Record<string, CatalogAction[]> = {};
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-900 dark:text-white">
            ...
            All catalog capabilities as actions
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {total} capabilities across the estate, products and KPIs — read-only ones run inline, changes open the Sources page
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search catalog capabilities — e.g. lineage, classify, product, KPI, clustering, refresh…"
          ...
          aria-label="Search catalog capabilities"
        />
      </div>
      ...
      {unavailable && !loading && (
        <p role="status" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-400">
          The catalog action registry isn&apos;t available on this account yet.
        </p>
      )}
      {error && !loading && (
