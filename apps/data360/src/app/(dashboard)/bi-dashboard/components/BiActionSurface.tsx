...
/**
 * BiActionSurface — the agentic command surface for BI Dashboard: every
 * dashboard capability rendered as a governed, verifiable action, sourced
 * live from the registry-in-tables catalog (GET /bi-dashboard/actions), not
 * from hardcoded buttons. Mirrors the other module Action surfaces.
 *
 * Read-only actions (probe='get', GET, no path params) → "Run" executes
 * inline. Everything else → "Open" jumps to the BI builder where the full
 * control lives (mutations need a selected dashboard + confirmations, so
...
import {
  ...
  isBiActionVerified,
  type BiAction,
  type BiActionCatalog,
} from '@/app/services/bi-dashboard/actions';

const AREA_META: Record<string, { label: string; hint: string }> = {
  build: { label: 'Build', hint: 'Pages, widgets, filters, templates, AI drafting' },
  data: { label: 'Data', hint: 'Chart queries, page render, drill-through' },
  manage: { label: 'Manage', hint: 'Design, versions, cost, export, lifecycle' },
  share: { label: 'Publish & share', hint: 'Publish/unpublish and share grants' },
};
const AREA_ORDER = ['build', 'data', 'manage', 'share'];
...
function ActionCard({
  ...
  action: BiAction;
  run?: RunState;
  onRun: (a: BiAction) => void;
  onOpen: (a: BiAction) => void;
}) {
  const runnable = action.probe === 'get' && action.method === 'GET' && !action.path.includes('{');
  const verified = isBiActionVerified(action.verified_status);
  return (
...
export default function BiActionSurface() {
  ...
  const { data, loading, error, unavailable, refetch } = useCacheAwareQuery<BiActionCatalog>(
    getBiActions,
    { cacheKeys: [CACHE_KEYS.CHARTS] },
  );

  const runAction = useCallback(async (a: BiAction) => {
    setRuns((r) => ({ ...r, [a.action_id]: { running: true } }));
    trackFeatureClick(`bi_action_run_${a.action_id}`);
    try {
  ...
  const openArea = useCallback((a: BiAction) => {
    trackFeatureClick(`bi_action_open_${a.action_id}`);
    router.push('/bi-dashboard');
    toast(`Use the BI builder to complete "${a.label}".`, { icon: '→' });
  }, [router, trackFeatureClick]);

  const runVerify = useCallback(async () => {
    setVerifying(true);
    trackFeatureClick('bi_actions_verify');
    try {
      const res = await verifyBiActions();
  ...
  const filtered = useMemo(() => {
    ...
    const rows = q
      ...
      : all;
    const byArea: Record<string, BiAction[]> = {};
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-900 dark:text-white">
            ...
            All BI-dashboard capabilities as actions
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {total} capabilities across build, data, lifecycle and sharing — read-only ones run inline, changes open the builder
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search dashboard capabilities — e.g. widget, publish, share, snapshot, drill, cost…"
          ...
          aria-label="Search dashboard capabilities"
        />
      </div>
      ...
      {unavailable && !loading && (
        <p role="status" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-400">
          The BI action registry isn&apos;t available on this account yet.
        </p>
      )}
      {error && !loading && (
