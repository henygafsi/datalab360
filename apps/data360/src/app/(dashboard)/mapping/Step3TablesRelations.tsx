...
// --- Custom Node Component with Enhancements ---
...
const Step3TablesRelations: React.FC<Step3Props> = ({
    ...
    selectedSourceTables, selectedTargetTable, projectId,
}) => {
    ...
    useEffect(() => {
        const initializeGraph = () => {
            if (!selectedTargetTable || !mappingData.column_attributes || Object.keys(mappingData.column_attributes).length === 0) {
                console.warn("Step3: Waiting for target table or column attributes to initialize.");
                setLoading(false);
                return;
            }
            setLoading(true);
            ...
            const processTable = (table: TableSelection, isSource: boolean) => {
                ...
                const columns: ColumnDetail[] = Object.entries(tableAttrs).map(([colName, attrs]) => {
                    const isPrimaryKey = isSource ? (pkSourceAttrs[tableKey] || []).includes(colName) : pkTargetAttrs.includes(colName);
                    return { name: colName, data_type: attrs.data_type || 'unknown', is_primary_key: isPrimaryKey, is_required_for_mapping: attrs.is_required_for_mapping || isPrimaryKey };
                });
                if (!tempColumnsMap[tableKey]) tempColumnsMap[tableKey] = {};
                columns.forEach(c => tempColumnsMap[tableKey][c.name] = c);
            ...
            const initialEdges: Edge[] = (mappingData.column_mappings || []).map((m, i) => ({
                ...
                target: `${selectedTargetTable.database}.${selectedTargetTable.schema}.${selectedTargetTable.table}`,
