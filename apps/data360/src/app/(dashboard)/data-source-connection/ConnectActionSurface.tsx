...
/**
 * ConnectActionSurface — the agentic command surface for Connect: every
 * ingestion capability rendered as a governed, verifiable action, sourced
 * live from the registry-in-tables catalog (GET /connect/actions), not from
 * hardcoded buttons. Mirrors the AI / E&D / Admin / Governance / Projects /
 * Workflow catalogs.
 *
 * Read-only actions (probe='get', GET, no path params) → "Run" executes
 * inline. Everything else → "Open" jumps to the connector cockpit where the
 * full wizard lives (mutations need credentials + confirmations, so nothing
 * mutating runs from the catalog directly).
 */
...
import {
  getConnectActions,
  verifyConnectActions,
  isConnectActionVerified,
  type ConnectAction,
  type ConnectActionCatalog,
} from '@/app/services/connect/actions';

const AREA_META: Record<string, { label: string; hint: string }> = {
  discover: { label: 'Discover & wizard', hint: 'Catalog, AI suggest, pre-flight checks, health, sync' },
  stage: { label: 'Stages & files', hint: 'Landing zones — upload, preview, load to table, grants' },
  cloud: { label: 'Cloud storage', hint: 'S3 / Azure / GCS integrations, stages, Snowpipe' },
  database: { label: 'Databases', hint: 'PostgreSQL, MySQL, Oracle ingest' },
  lakehouse: { label: 'Lakehouse', hint: 'Databricks, Iceberg, Snowflake datalake' },
  api: { label: 'APIs', hint: 'REST ingest and open-data presets' },
  pipeline: { label: 'Pipelines', hint: 'Ingest task suspend / resume' },
};
const AREA_ORDER = ['discover', 'stage', 'cloud', 'database', 'lakehouse', 'api', 'pipeline'];
...
function ActionCard({
  ...
  run?: RunState;
  onRun: (a: ConnectAction) => void;
  onOpen: (a: ConnectAction) => void;
}) {
  const runnable = action.probe === 'get' && action.method === 'GET' && !action.path.includes('{');
  const verified = isConnectActionVerified(action.verified_status);
  return (
...
export default function ConnectActionSurface() {
  ...
  const { data, loading, error, unavailable, refetch } = useCacheAwareQuery<ConnectActionCatalog>(
    getConnectActions,
    { cacheKeys: [CACHE_KEYS.CONNECTORS] },
  );

  const runAction = useCallback(async (a: ConnectAction) => {
    setRuns((r) => ({ ...r, [a.action_id]: { running: true } }));
    trackFeatureClick(`connect_action_run_${a.action_id}`);
    try {
  ...
  const openArea = useCallback((a: ConnectAction) => {
    trackFeatureClick(`connect_action_open_${a.action_id}`);
    router.push(routes.connexion.dataSourceConnection);
    toast(`Use the connector cockpit to complete "${a.label}".`, { icon: '→' });
  }, [router, trackFeatureClick]);

  const runVerify = useCallback(async () => {
    setVerifying(true);
    trackFeatureClick('connect_actions_verify');
    try {
      const res = await verifyConnectActions();
  ...
  const filtered = useMemo(() => {
    ...
    const rows = q
      ...
      : all;
    const byArea: Record<string, ConnectAction[]> = {};
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-900 dark:text-white">
            ...
            All ingestion capabilities as actions
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {total} capabilities across sources, stages and pipelines — read-only ones run inline, changes open the cockpit
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search ingestion capabilities — e.g. upload, Snowpipe, sync, preview, Iceberg, grants…"
          ...
          aria-label="Search connect capabilities"
        />
      </div>
      ...
      {unavailable && !loading && (
        <p role="status" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-400">
          The connect action catalog isn&apos;t available on this account yet.
        </p>
      )}
      {error && !loading && (
