  ...
  const handleAzureSubmit = async (e: FormEvent) => {
      ...
      try {
          if (azureCurrentSubStep === 1) { // Step 1: Create Storage Integration
              ...
              setAzureConsentUrl(response?.details?.azure_consent_url || null);
              setAzureMultiTenantAppName(response?.details?.azure_multi_tenant_app_name || null);
          ...
          <div className="mx-auto w-full max-w-lg transform rounded-2xl bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl p-10 shadow-xl border border-slate-200/50 dark:border-slate-700/50">
              ...
              <form className="space-y-6" onSubmit={handleAzureSubmit}>
                  ...
                  {azureCurrentSubStep === 2 && (
                      <>
                          <h4 className="text-xl font-medium text-gray-800 dark:text-gray-200">Step 2: Notification Integration (Optional)</h4>
                  ...
                  {azureCurrentSubStep === 3 && showAzureNotificationOption && azureNotificationIntegrationCreated && (
                      <>
                          <h4 className="text-xl font-medium text-gray-800 dark:text-gray-200">Step 3: Azure Consent Details</h4>
                          ...
                          {azureNotificationDetailsFetched && (
                              <Button type="button" onClick={() => setAzureCurrentSubStep(4)} className="w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 shadow-elevation-2 hover:shadow-elevation-3 transition-all duration-300 transform hover:scale-[1.02]" disabled={loading}>
                  ...
                  {azureCurrentSubStep === 4 && (
                      <>
                          <h4 className="text-xl font-medium text-gray-800 dark:text-gray-200">Step {showAzureNotificationOption ? '4' : '3'}: Create Azure Stage</h4>
                          <Input
                              name="stage_name"
                              label="Stage Name"
                ...
                case 'azure':
                    return (
                        <div className="mx-auto max-w-2xl space-y-8">
                            <Breadcrumb />
                            <StepIndicator currentStep={azureCurrentSubStep} totalSteps={showAzureNotificationOption ? 4 : 3} />
                            {renderAzureForm()}
                        </div>
                    );
                case 'aws':
