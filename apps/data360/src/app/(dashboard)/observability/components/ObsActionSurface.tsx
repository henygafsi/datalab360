...
/**
 * ObsActionSurface — the agentic command surface for Observability: every
 * operator capability rendered as a governed, verifiable action, sourced
 * live from the registry-in-tables catalog (GET /observability/actions),
 * not from hardcoded buttons. Mirrors the AI / E&D / Admin / Governance /
 * Projects / Workflow / Connect / DQ / Catalog surfaces.
 *
 * Read-only actions (probe='get', GET, no path params) → "Run" executes
 * inline. Everything else → "Open" jumps to the Observability page where the
 * full control lives (mutations need context + confirmations, so nothing
...
import {
  ...
  isObsActionVerified,
  type ObsAction,
  type ObsActionCatalog,
} from '@/app/services/observability/actions';

const AREA_META: Record<string, { label: string; hint: string }> = {
  pulse: { label: 'Pulse', hint: 'KPIs, dashboard, activity, sensors, SLO tracking' },
  cost: { label: 'Cost (FinOps)', hint: 'Spend reads, resource monitors, budgets' },
  perf: { label: 'Performance', hint: 'Metrics, slow queries, exports' },
  lineage: { label: 'Lineage', hint: 'Lineage graphs, access patterns, dependencies' },
  alert: { label: 'Alerts & SLOs', hint: 'Incident feed, acknowledge, SLO definitions' },
  comply: { label: 'Compliance', hint: 'GDPR, SOC2, security posture, Trust Center' },
  probe: { label: 'Freshness probes', hint: 'Table / schema / platform freshness checks' },
};
const AREA_ORDER = ['pulse', 'cost', 'perf', 'lineage', 'alert', 'comply', 'probe'];
...
function ActionCard({
  ...
  action: ObsAction;
  run?: RunState;
  onRun: (a: ObsAction) => void;
  onOpen: (a: ObsAction) => void;
}) {
  const runnable = action.probe === 'get' && action.method === 'GET' && !action.path.includes('{');
  const verified = isObsActionVerified(action.verified_status);
  return (
...
export default function ObsActionSurface() {
  ...
  const { data, loading, error, unavailable, refetch } = useCacheAwareQuery<ObsActionCatalog>(
    getObsActions,
    { cacheKeys: [CACHE_KEYS.OBSERVABILITY_DASHBOARD] },
  );

  const runAction = useCallback(async (a: ObsAction) => {
    setRuns((r) => ({ ...r, [a.action_id]: { running: true } }));
    trackFeatureClick(`obs_action_run_${a.action_id}`);
    try {
  ...
  const openArea = useCallback((a: ObsAction) => {
    trackFeatureClick(`obs_action_open_${a.action_id}`);
    router.push('/observability');
    toast(`Use the Observability page to complete "${a.label}".`, { icon: '→' });
  }, [router, trackFeatureClick]);

  const runVerify = useCallback(async () => {
    setVerifying(true);
    trackFeatureClick('obs_actions_verify');
    try {
      const res = await verifyObsActions();
  ...
  const filtered = useMemo(() => {
    ...
    const rows = q
      ...
      : all;
    const byArea: Record<string, ObsAction[]> = {};
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-900 dark:text-white">
            ...
            All observability capabilities as actions
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {total} capabilities across cost, performance and reliability — read-only ones run inline, changes open the page
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search observability capabilities — e.g. budget, monitor, slow queries, lineage, SLO, GDPR…"
          ...
          aria-label="Search observability capabilities"
        />
      </div>
      ...
      {unavailable && !loading && (
        <p role="status" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-400">
          The observability action registry isn&apos;t available on this account yet.
        </p>
      )}
      {error && !loading && (
