...
/**
 * DataQualityActionSurface — the agentic command surface for Data Quality:
 * every steward capability rendered as a governed, verifiable action, sourced
 * live from the registry-in-tables catalog (GET /data-quality/actions), not
 * from hardcoded buttons. Mirrors the AI / E&D / Admin / Governance /
 * Projects / Workflow / Connect catalogs.
 *
 * Read-only actions (probe='get', GET, no path params) → "Run" executes
 * inline. Everything else → "Open" jumps to the DQ page where the full
 * control lives (mutations need a table + confirmations, so nothing mutating
 * runs from the catalog directly).
 */
...
import {
  getDataQualityActions,
  verifyDataQualityActions,
  isDataQualityActionVerified,
  type DataQualityAction,
  type DataQualityActionCatalog,
} from '@/app/services/data-quality/actions';

const AREA_META: Record<string, { label: string; hint: string }> = {
  check: { label: 'Checks', hint: 'Run checks, DMF suggestions, auto-profile, optimize' },
  dmf: { label: 'Metric lifecycle', hint: 'Attach DMFs, schedules, thresholds, breaches, inventory' },
  monitor: { label: 'Monitors', hint: 'Completeness, freshness, uniqueness, ingestion, schema, security' },
  anomaly: { label: 'Anomalies & Trust', hint: 'ML detectors and Snowflake Trust Center' },
  report: { label: 'Reports', hint: 'Quality score, trends, run history, full snapshot' },
};
const AREA_ORDER = ['check', 'dmf', 'monitor', 'anomaly', 'report'];
...
function ActionCard({
  ...
  run?: RunState;
  onRun: (a: DataQualityAction) => void;
  onOpen: (a: DataQualityAction) => void;
}) {
  const runnable = action.probe === 'get' && action.method === 'GET' && !action.path.includes('{');
  const verified = isDataQualityActionVerified(action.verified_status);
  return (
...
export default function DataQualityActionSurface() {
  ...
  const { data, loading, error, unavailable, refetch } = useCacheAwareQuery<DataQualityActionCatalog>(
    getDataQualityActions,
    { cacheKeys: [CACHE_KEYS.DATA_QUALITY] },
  );

  const runAction = useCallback(async (a: DataQualityAction) => {
    setRuns((r) => ({ ...r, [a.action_id]: { running: true } }));
    trackFeatureClick(`dq_action_run_${a.action_id}`);
    try {
  ...
  const openArea = useCallback((a: DataQualityAction) => {
    trackFeatureClick(`dq_action_open_${a.action_id}`);
    router.push('/data-quality');
    toast(`Use the Data Quality page to complete "${a.label}".`, { icon: '→' });
  }, [router, trackFeatureClick]);

  const runVerify = useCallback(async () => {
    setVerifying(true);
    trackFeatureClick('dq_actions_verify');
    try {
      const res = await verifyDataQualityActions();
  ...
  const filtered = useMemo(() => {
    ...
    const rows = q
      ...
      : all;
    const byArea: Record<string, DataQualityAction[]> = {};
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-900 dark:text-white">
            ...
            All quality capabilities as actions
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {total} capabilities across checks, metrics and monitors — read-only ones run inline, changes open the DQ page
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search quality capabilities — e.g. threshold, freshness, breach, anomaly, profile, trend…"
          ...
          aria-label="Search quality capabilities"
        />
      </div>
      ...
      {unavailable && !loading && (
        <p role="status" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-400">
          The data-quality action catalog isn&apos;t available on this account yet.
        </p>
      )}
      {error && !loading && (
