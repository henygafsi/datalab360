...
import PolicyBuilderSection from './PolicyBuilderSection';
...
// Policy types that can be applied ('builder' = guided New-policy flow
// covering the full Snowflake family: masking, RLS, aggregation, projection,
// join, tag-based — object-first, queued as traced release changes)
export type PolicyCategory = 'builder' | 'masking' | 'rls' | 'tags' | 'aggregation';
...
const PolicyTab: React.FC<{
  ...
  <button
    className={cn(
      // Compact scale (13" @100% zoom): matches the release-tab section
      // headers so every right-bar tab reads the same.
  ...
  >
    ...
    {count !== undefined && count > 0 && (
      <Badge className="ml-1 bg-blue-500 text-white text-[10px]">{count}</Badge>
    )}
  </button>
);
...
const PolicyAssignmentPanel: React.FC<PolicyAssignmentPanelProps> = ({
  ...
  // Guided builder first: the panel opens on "New" (object-first creation)
  // unless a deep-link forces a specific apply tab.
  const [activeTab, setActiveTab] = useState<PolicyCategory>(initialCategory ?? 'builder');
    ...
    return (
      <div className={cn('p-4 text-center', className)}>
        ...
        <Text className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
          Every Snowflake policy attaches to a table, a view or a column —
          select a table on the canvas, then choose the policy type here.
        </Text>
      </div>
    );
  }
    ...
    <div className={cn('flex flex-col h-full', className)}>
      ...
      <div className="px-3 py-2 border-b dark:border-slate-700 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <Text className="text-xs font-semibold">{table.table}</Text>
            <Text className="text-[10px] text-slate-500">{table.database}.{table.schema}</Text>
          </div>
        </div>
        {onClose && (
          <button
      ...
      {/* Tabs */}
      <div className="px-3 py-1.5 border-b dark:border-slate-700 flex gap-1 overflow-x-auto">
        <PolicyTab
          category="builder"
          label="New"
          icon={Plus}
      ...
      <div className="flex-1 overflow-auto p-3">
        {activeTab === 'builder' && (
          <PolicyBuilderSection
            ...
            projectId={projectId}
            onDrafted={(policyName) =>
              handlePolicyApplied({
                type: 'builder',
                name: policyName,
                target: table ? `${table.schema}.${table.table}` : '',
                details: 'queued for release',
              })
            }
          />
        )}
