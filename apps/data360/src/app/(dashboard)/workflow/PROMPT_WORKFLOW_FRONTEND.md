# Data360 — Workflow Module API Reference
...
The workflow engine builds a **DAG of components** (steps). Each step has:
...
At execution, the engine **topologically sorts** the DAG and compiles all steps into a single SQL `WITH ... AS` chain. The `destination` step materializes the result into a physical Snowflake table.
...
**StepStatus**: `waiting`, `running`, `completed`, `failed`, `skipped`
...
```json
{
  ...
  "metadata": { "initial_steps_count": 2 },
  "tags": ["etl", "customers"]
}
```
