...
import {
  ...
  Beaker, ChevronRight, ServerCrash, Lock,
} from 'lucide-react';
...
export default function WizardPreflightPanel({
  ...
  const phases: Phase[] = [
    ...
    // 2. Source data clone
    {
      ...
      goal: 'Fetch a 5-row sample of every source block from Snowflake to confirm the bindings before deploy.',
    ...
    // 4. Compiled code
    {
      id: 'codegen',
      title: 'Compiled code',
      goal: 'SQL, Python and YAML snippets generated by Cortex for inspection. Optional but useful for review.',
      status:
    ...
    // 5. Server dry-run — backend gap
    {
      ...
      goal: 'Compile + execute the workflow against Snowflake with a row limit, without touching destination tables.',
      status: 'backend_gap',
      detail: (
        <BackendGapNote
