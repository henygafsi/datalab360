...
/**
 * CcActionSurface — the agentic command surface for Command-Center /
 * Account-Overview: every executive capability rendered as a governed,
 * verifiable action, sourced live from the registry-in-tables catalog
 * (GET /command-center/actions), not from hardcoded buttons. Mirrors the
 * AI / E&D / Admin / Governance / Projects / Workflow / Connect / DQ /
 * Catalog / Observability surfaces.
 *
 * Read-only actions (probe='get', GET, no path params) → "Run" executes
 * inline — with ACCOUNTADMIN these are the whole-account audit/cost/infra
 * sweeps. Everything else → "Open" jumps to Account Overview where the full
 * control lives.
 */
...
import {
  ...
  isCcActionVerified,
  type CcAction,
  type CcActionCatalog,
} from '@/app/services/command-center/actions';

const AREA_META: Record<string, { label: string; hint: string }> = {
  exec: { label: 'Executive', hint: 'Summary, module health, KPI scorecards, tab payloads' },
  audit: { label: 'Audit trails', hint: 'Query, access and login history, security intelligence' },
  cost: { label: 'Cost intelligence', hint: 'Warehouse/service/clustering/pipe/MV/task/storage spend' },
  infra: { label: 'Infrastructure', hint: 'Warehouses, pipelines, queries, lineage, roles' },
  reco: { label: 'Recommendations', hint: 'Actionable recos + the DWH improvement proposal' },
  project: { label: 'Project rollups', hint: 'Per-project scores and KPI rollups' },
  ops: { label: 'Cache ops', hint: 'Warm caches, refresh and provision overview KPIs' },
};
const AREA_ORDER = ['exec', 'audit', 'cost', 'infra', 'reco', 'project', 'ops'];
...
function ActionCard({
  ...
  action: CcAction;
  run?: RunState;
  onRun: (a: CcAction) => void;
  onOpen: (a: CcAction) => void;
}) {
  const runnable = action.probe === 'get' && action.method === 'GET' && !action.path.includes('{');
  const verified = isCcActionVerified(action.verified_status);
  return (
...
export default function CcActionSurface() {
  ...
  const { data, loading, error, unavailable, refetch } = useCacheAwareQuery<CcActionCatalog>(
    getCcActions,
    { cacheKeys: [CACHE_KEYS.OBSERVABILITY_DASHBOARD] },
  );

  const runAction = useCallback(async (a: CcAction) => {
    setRuns((r) => ({ ...r, [a.action_id]: { running: true } }));
    trackFeatureClick(`cc_action_run_${a.action_id}`);
    try {
  ...
  const openArea = useCallback((a: CcAction) => {
    trackFeatureClick(`cc_action_open_${a.action_id}`);
    router.push('/account-overview');
    toast(`Use Account Overview to complete "${a.label}".`, { icon: '→' });
  }, [router, trackFeatureClick]);

  const runVerify = useCallback(async () => {
    setVerifying(true);
    trackFeatureClick('cc_actions_verify');
    try {
      const res = await verifyCcActions();
  ...
  const filtered = useMemo(() => {
    ...
    const rows = q
      ...
      : all;
    const byArea: Record<string, CcAction[]> = {};
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <h2 className="flex items-center gap-1.5 text-sm font-semibold text-slate-900 dark:text-white">
            ...
            All account-overview capabilities as actions
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {total} capabilities across the whole account — read-only sweeps run inline with your role's real access
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search account capabilities — e.g. audit, storage, role hierarchy, recommendations, rollup…"
          ...
          aria-label="Search account capabilities"
        />
      </div>
      ...
      {unavailable && !loading && (
        <p role="status" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-800/40 dark:text-slate-400">
          The command-center action registry isn&apos;t available on this account yet.
        </p>
      )}
      {error && !loading && (
