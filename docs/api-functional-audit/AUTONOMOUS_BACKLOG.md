...
- [ ] **AVANT autres batchs** : valider manuellement les vrais gaps par module (le détecteur sur-compte). Option suivi : raffiner `gen-api-catalog` (match préfixes de templates) — prudent (éviter faux-wired).
- [ ] Batchs suivants (vrais gaps seulement, par module) : explore-design, gouvernance, org-accounts… 1 batch validé/vague.
