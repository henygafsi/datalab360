...
- [x] Suivi FAIT : 5 pages `fallback={null}` (data360-config, governance projects/policies, sources, bi-dashboard) → composant partagé `RouteFallback` (skeleton). Build VERT, commit da6b040.
...
- [x] **Batch 3 — `/explore-design/glossary/*`** (5 endpoints CRUD+AI, validé absent) : groupe glossary dans exploreDesign + service `services/glossary`. wired 503→508. Build VERT, commit 3213749.
- [x] **Batch 4 — dynamic-tables/streams/tasks** (19 endpoints, 3 clusters Snowflake-objects validés absents) : contrat + service `snowflake-objects`. wired 508→522. Build VERT, commit bdc1a22. Note : 5 endpoints d'action (suspend/resume/refresh) fonctionnellement wirés mais comptés conservativement non-wirés (segment action interpolé).
- [x] **Batch 5 — gouvernance identité & intégrations** (17 endpoints : oauth 8, gui-permissions 5, enterprise-users 4, validés absents) + service `governance/identity-integrations`. wired 522→539. Build VERT, commit 749c2cd.
- [ ] Batchs suivants : explore-design ({project_id} 54, guided 15…), gouvernance policies 51, org-accounts 68… 1 cluster validé/vague.
...
### W8 — Paiement UI + dry-run gratuit (sample data) + coût maîtrisé — FRONT ⏳ scaffold
- [x] **Primitive `DryRunGate`** (`shared/insights`) : wrappe une action coûteuse → *dry-run gratuit (sample, 0 crédit)* vs *run réel (coût estimé + confirm + ack paiement)*. Réutilise InsightActionButton (confirm/toast/gate). Build VERT, commit abfca73. **Intégration paiement = TODO (décision provider).** Prêt à adopter, pas encore branché sur une action live.
- [ ] Adopter `DryRunGate` sur 1-2 actions coûteuses réelles (ex. data-quality optimize/recluster, cache warmup) — nécessite que le backend expose un flag `dryRun`.
- [ ] Brancher le paiement réel quand le provider est choisi.
...
|---|---|---|
| 2026-06-29 13:4x CEST | Diagnostic costGov 404 finalise (read-only, no commit) | Trace : CostGovernancePanel l.393 lit account=session.user.account_name=uchsfvb-HAHA (login org-qualifie) passe a /org-accounts/warehouses/{account_name} qui attend HAHA (nom Snowflake). = bug FE identifiant compte, pas un 1-ligne aveugle (normaliser/CURRENT_ACCOUNT, valider). Reporte dans HAHA_FUNCTIONAL_AUDIT. QUESTION OUVERTE : autorises-tu les commits pour appliquer les fixes du rapport ? Sinon je reste en hold. |
| 2026-06-29 13:0x CEST | Données HAHA 100% (dernier bloqueur fixé, env-only) | Audit fonctionnel des 4 modules livré (HAHA_FUNCTIONAL_AUDIT_2026-06-29.md). Dernier bloqueur data resolu SANS code/commit/deploy : flag dev CACHE_READ_ALLOW_USER_FALLBACK=1 dans backend/app/.env → admin-events bascule sur le SVC → usage-by/endpoint-usage = 200 (etaient 500), access-center/admin-performance rendent. Reste 2 mineurs (warehouse 404, perf cross-account 403 = RBAC correct). |
...
| 2026-06-29 08:36 CEST | W6 (org FinOps) + 2e finding | Réintégré 3 vrais gaps FinOps org (`organization/costs`/`storage`/`remaining-balance`) + service `org-accounts/finops`. wired 539→541. **2e classe de faux positifs trouvée** : le détecteur rate aussi les templates **backticks imbriqués** (credits/trend etc. déjà wirés mais comptés non) → vrai non-wiré encore < compteur. Caveat documenté, détecteur NON re-touché (risque faux-wired). Build VERT, commit 9d8ded9. |
| 2026-06-29 08:50 CEST | Test live Snowflake org→HAHA | PAT ORGADMIN read-only : hiérarchie org→HAHA réelle (HAHA + KY11038 + autres) ; FinOps data RÉELLE (org costs KY11038 $677/HAHA $482, credits HAHA 135.33, remaining -1097 USD). **Preuve : pages vides/503 = backend déployé down, PAS features manquantes.** 2 fixes backend trouvés (ORGANIZATION_USAGE vs SHOW vide ; COMPUTE_WAH ; col storage). Rapport SNOWFLAKE_VALIDATION_org_to_haha.md. PAT à roter. |
| 2026-06-29 08:06 CEST | W8 scaffold (DryRunGate) | Créé le primitive FinOps `DryRunGate` (shared/insights) : dry-run gratuit sample vs run réel cost-gated + ack paiement (réutilise InsightActionButton). Build VERT, commit abfca73. Paiement réel = TODO (décision provider) ; primitive prêt à adopter. Avancé W8 sans bloquer sur la décision provider. |
| 2026-06-29 07:41 CEST | Vérif intégrité (re-sweep) | Re-sweep Playwright après 35+ commits : **43/64 routes (kill externe avant la fin), 0 non-ok** — aucune régression. Ex-crashers client-accounts/ai-advisor/snowpark = **ok** (fixes W1 tiennent ; erreurs API restantes = backend-cause). costGov/administration non atteints mais couverts par même fix. Note : dev server + sweep tués ensemble (stop externe) → relancé dev. Pas de code, pas de commit. |
| 2026-06-29 07:06 CEST | W6 (batch 5 — gov identité) | Réintégré gouvernance identité & intégrations : oauth (8) + gui-permissions (5) + enterprise-users (4) = 17 endpoints validés absents → contrat + service `governance/identity-integrations`. wired 522→539, unwired →481, build VERT, commit 749c2cd. Bilan W6 : 5 batchs, +56 endpoints. Prochaine : autre cluster ou W7/W8/W9 (à décisions). |
| 2026-06-29 06:35 CEST | W4 suivi (5 fallbacks) | Durci 5 pages `fallback={null}` → composant partagé `RouteFallback` (skeleton au lieu de blank sur suspension : data360-config, governance projects/policies, sources, bi-dashboard). Build VERT, commit da6b040. Prochaine : réintégration ou W8/W9 (à décisions). |
| 2026-06-29 06:07 CEST | Polish vue fonctionnelle | Enrichi la vue (le livrable phare) : **détail par ligne** au clic (desc complète, audience, RBAC, cache, IA, FinOps, wiré, tags, params path/query/body, réponse stockée). Build VERT + **vérifié runtime** (clic → ligne détail s'affiche, RBAC visible). Commit 0f11808. Choix : enrichir le livrable plutôt que d'empiler des batchs de wiring invisibles. |
| 2026-06-29 05:35 CEST | W6 (batch 4 — SF objects) | Réintégré 3 clusters cohérents `dynamic-tables`/`streams`/`tasks` (19 endpoints, validés absents) → contrat + service `snowflake-objects`. wired 508→522, unwired →498, build VERT, commit bdc1a22. Note honnête : 5 endpoints d'action comptés conservativement non-wirés (interpolation). Prochaine : autre cluster ou W8/W9. |
| 2026-06-29 05:05 CEST | W6 (batch 3 — glossary) | Réintégré le cluster cohérent `/explore-design/glossary/*` (5 endpoints CRUD+AI, validé genuinely-unwired) → groupe glossary + service typé `services/glossary` (aide clarté data_user). wired 503→508, build VERT, commit 3213749. Prochaine : autre cluster explore-design (dynamic-tables/streams/tasks) ou gouvernance, ou W8/W9. |
